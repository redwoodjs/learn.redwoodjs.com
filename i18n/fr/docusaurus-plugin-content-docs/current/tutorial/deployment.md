...
> Si vous déployez sur Netlify et utilisez la version Prisma `< 2.11.`, vous devez ajouter `rhel-openssl-1.0.x` à vos `binaryTargets`:
> 
> ```javascript
...
N’oubliez pas de cliquer sur le bouton **Enregistrer**. Maintenant, allez à l'onglet **Déploie** en haut et ouvrez la **liste** sur la droite. puis choisissez enfin **"Deploy site"**:
...
Nous travaillons à faciliter ce processus, mais gardez tout de même cela à l'esprit avant de déployer une application Redwood en production et la présenter au monde entier.
