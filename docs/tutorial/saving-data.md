...
If you'll [recall](https://learn-redwood.netlify.app/docs/tutorial/everyones-favorite-thing-to-build-forms/#onsubmit) `<Form>` gives us all of the fields in a nice object where the key is the name of the field, which means the `data` object we're receiving in `onSubmit` is already in the proper format that we need for the `input`!
