...
As for our blog, we're going to use self-hosted authentication (named *dbAuth* in Redwood) since it's the simplest and doesn't involve any third party signups.
...
* **hashedPassword**: stores the result of combining the user's password with a `salt`. then hashed
...
Going to the admin section now prevents a non-logged in user from seeing posts. Great! This is the result of the `@requireAuth` directive in `api/src/graphql/posts.sdl`. But, ideally they wouldn't be able to see the admin at all. Let's fix that with a new component in the Routes file, `<Private>`:
...
Do you know why we're seeing the same message here that we saw in the posts admin page?
...
Now that we're logged in, how do we log out? Let's a link to the `BlogLayout` so that it's present on all pages, and also include an indicator of who you're actually logged in as.
...
Well, it's almost right! Where's our email address? By default, the function that determines what's in `currentUser` only returns that user's `id` field for security reasons (better to expose too little than too much!). To add email to that list, check out `api/src/lib/auth.js`:
