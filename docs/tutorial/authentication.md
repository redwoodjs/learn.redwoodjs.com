...
It's because the `posts` query in `posts.sdl` is used by both the homepage _and_ the posts admin page. Since it has the `@requireAuth` directive, it's locked down and can only be accessed when logged in. But we _do_ want people that aren't logged in to be able to view the posts on the homepage!
